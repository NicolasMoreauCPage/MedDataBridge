"""Runner helpers to start/stop endpoint background workers.

This module exposes a RunnerRegistry and a small helper to register the
ASGI server's running asyncio event loop so synchronous request handlers can
schedule coroutines safely using ``asyncio.run_coroutine_threadsafe``.
"""
...
def set_event_loop(loop: asyncio.AbstractEventLoop) -> None:
    """Register the ASGI server's running loop for thread-safe scheduling.

    This should be called from the application's startup (lifespan) when a
    running loop is available.
    """
        ...
        if endpoint.kind == "MLLP" and endpoint.role in ("receiver", "both"):
            ...
            if _main_event_loop is not None:
                asyncio.run_coroutine_threadsafe(
                    manager.start_endpoint(endpoint), _main_event_loop
                )
            else:
                try:
...
__all__ = ["RunnerRegistry", "manager", "registry", "set_event_loop"]
