...
class HprimAcquittementService:
    ...
    def process_acquittement(
        self, acquittement_data: Dict[str, Any]
        ...
        try:
            ...
            acquittement = Acquittement(
                ...
                erreurs="\n".join(str(e) for e in acquittement_data.get("erreurs", []))
                or None,
                avertissements="\n".join(
                    str(a) for a in acquittement_data.get("avertissements", [])
                )
                or None,
            )
            ...
            for resp in acquittement_data.get("reponses_interventions", []):
                self._add_reponse(
                    ...
                    resp,
                    "INTERVENTION",
                    identifiant_key="identifiant_intervention",
                    code_key="code_intervention",
                )
    ...
    def get_acquittement_by_message_id(self, message_id: str) -> Optional[Acquittement]:
    ...
    def get_acquittement_status_summary(self, message_id: str) -> Dict[str, Any]:
        """Retourne un résumé du statut des actes dans l'acquittement"""
        acquittement = self.get_acquittement_by_message_id(message_id)
        if not acquittement:
            return {}

        reponses = self.session.exec(
            select(AcquittementReponse).where(
                AcquittementReponse.acquittement_id == acquittement.id
            )
        ).all()
    ...
    def list_recent_acquittements(self, limit: int = 50) -> List[Acquittement]:
