...
def create_intervention(
    ...
    try:
        ...
        intervention = service.create_intervention(
            dossier_id=dossier_id, **payload.model_dump()
        )
...
def get_dossier_cotations_count(
    dossier_id: int, session: Session = Depends(get_session)
) -> dict:
    """
    ...
    try:
        service = HprimInterventionService(session)
        count = service.get_dossier_cotations_count(dossier_id)
...
def list_interventions(
    ...
    try:
        service = HprimInterventionService(session)
        interventions = service.get_interventions_for_dossier(dossier_id)
...
def get_intervention_cotations(
    intervention_id: int, session: Session = Depends(get_session)
) -> dict:
    """
    ...
    try:
        service = HprimInterventionService(session)
        cotations = service.get_cotations_for_intervention(intervention_id)
...
def update_cotations_flags(
    ...
    try:
        service = HprimInterventionService(session)
        success = service.update_dossier_cotations_flags(dossier_id)
