...
```
**Résultat attendu** : ~61 packages dans `deployment/dependencies/` (~27 MB)

### 1.2 Préparation du package
```bash
./prepare_deployment.sh
```
**Résultat attendu** : Application copiée dans `deployment/app/`

### 1.3 Création de l'archive
```bash
cd ../..
tar -czf meddata-bridge-deploy-$(date +%Y%m%d).tar.gz deployment/
```
...
# Copier la config
sudo cp /tmp/deployment/config/nginx-meddata-bridge.conf /etc/nginx/conf.d/
