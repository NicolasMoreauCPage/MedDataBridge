...
def test_edit_mouvement_page_renders(client, session: Session):
    ...
    d = Dossier(dossier_seq=int(uuid.uuid4().hex[:8], 16) % 1000000, patient_id=p.id, uf_responsabilite="UF-TEST", admit_time=datetime.now(), dossier_type=DossierType.HOSPITALISE)
    session.add(d); session.commit(); session.refresh(d)

    v = Venue(venue_seq=int(uuid.uuid4().hex[:8], 16) % 1000000, dossier_id=d.id, uf_responsabilite="UF-TEST", start_time=datetime.now())
    session.add(v); session.commit(); session.refresh(v)

    m = Mouvement(mouvement_seq=int(uuid.uuid4().hex[:8], 16) % 1000000, venue_id=v.id, when=datetime.now(), movement_type="admission", trigger_event="A01")
