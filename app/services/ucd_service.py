...
class UCDService:
    ...
    def create_act(self, act_data: UCDActCreate) -> UCDActResponse:
    ...
    def get_acts_by_dossier(self, dossier_id: int) -> List[UCDActResponse]:
    ...
    def update_act(self, act_id: int, act_data: UCDActUpdate) -> UCDActResponse:
    ...
    def get_act_by_id(self, act_id: int) -> UCDActResponse:
    ...
    def validate_act(self, act_id: int) -> UCDActResponse:
        """Valider un acte UCD"""
        act = self.db.get(UCDAct, act_id)
        if not act:
            raise HTTPException(status_code=404, detail="Acte UCD non trouvé")
