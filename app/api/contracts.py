...
class ContractCreate(BaseModel):
    ...
    def validate_contract_type(cls, v):
        if v not in ["NGAP", "UCD", "LPP"]:
            raise ValueError(
                "Type de contrat invalide. Valeurs autorisées: NGAP, UCD, LPP"
            )
        return v
    ...
    def validate_status(cls, v):
        if v not in ["actif", "suspendu", "termine"]:
            raise ValueError(
                "Statut invalide. Valeurs autorisées: actif, suspendu, termine"
            )
        return v
...
def create_contract(contract: ContractCreate, db: Session = Depends(get_session)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erreur lors de la création du contrat: {str(e)}"
        )


@router.get("/dossier/{dossier_id}", response_model=List[ContractResponse])
def get_contracts_by_dossier(dossier_id: int, db: Session = Depends(get_session)):
...
def get_active_contracts_by_dossier(
    dossier_id: int,
    ...
    try:
        contracts = ContractService.get_active_contracts_for_dossier(
            db, dossier_id, contract_type
        )
        return [ContractResponse.model_validate(contract) for contract in contracts]
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Erreur lors de la récupération des contrats actifs: {str(e)}",
        )


@router.put("/{contract_id}/status", response_model=ContractResponse)
def update_contract_status(
    contract_id: int, status: str, db: Session = Depends(get_session)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erreur lors de la mise à jour du statut: {str(e)}"
        )
...
def get_contract_coverage(
    ...
    try:
        contract = ContractService.get_contract_coverage(
            db, dossier_id, contract_type, act_date
        )
        if contract:
            return {
                "covered": True,
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Erreur lors de la vérification de couverture: {str(e)}",
        )


@router.post("/calculate-reimbursement")
def calculate_reimbursement(
    contract_id: int, act_amount: float, db: Session = Depends(get_session)
):
    ...
    try:
        ...
        reimbursed_amount = ContractService.calculate_reimbursement_amount(
            contract, act_amount
        )
        return {
        ...
        raise HTTPException(
            status_code=500, detail=f"Erreur lors du calcul du remboursement: {str(e)}"
        )
