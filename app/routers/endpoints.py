...
def _parse_optional_form_id(value: str | None, field_name: str) -> int | None:
    ...
    try:
        ...
        logger.warning(
            "Invalid endpoint form identifier field=%s value=%r", field_name, value
        )
        raise HTTPException(
            status_code=422,
...
def list_endpoints(request: Request, session=Depends(get_session), admin: bool = False):
    ...
    stmt = sqlmodel_select(SystemEndpoint).options(
        selectinload(SystemEndpoint.ght_context).selectinload(
            GHTContext.entites_juridiques
        ),
        selectinload(SystemEndpoint.entite_juridique).selectinload(
            EntiteJuridique.ght_context
        ),
    )
    ...
    if not admin:
        if ej_context_id:
            ...
            db_ght_context = (
                session.get(GHTContext, ght_context.id) if ght_context else None
            )
            ej_ids = (
                [ej.id for ej in db_ght_context.entites_juridiques]
                if db_ght_context
                else []
            )
            ...
            stmt = stmt.where(
                or_(
                    SystemEndpoint.ght_context_id == db_ght_context.id
                    if db_ght_context
                    else False,
    ...
    for e in eps:
        ...
        if not ght and e.entite_juridique:
            ...
            if (
                hasattr(e.entite_juridique, "ght_context")
                and e.entite_juridique.ght_context
            ):
    ...
    for ght_id in sorted(ght_groups.keys()):
        ...
        for ej_id in sorted(ght_data["ej_groups"].keys()):
            ...
            ght_children.append(
                {"type": "ej", "ej": ej_info["ej"], "endpoints": ej_endpoints}
            )

        # Endpoints without EJ in this GHT
        for e in ght_data["no_ej_endpoints"]:
            ght_children.append({"type": "endpoint", "endpoint": _make_endpoint_row(e)})

        hierarchy.append(
            {"type": "ght", "ght": ght_data["ght"], "children": ght_children}
        )
    ...
    ctx = {
        ...
        "is_admin": admin,
        "is_filtered": not admin
        and (ght_context is not None or ej_context_id is not None),
    }
    return get_templates_with_filters(request).TemplateResponse(
        request, "endpoints_hierarchical.html", ctx
    )
...
def new_endpoint(request: Request, session=Depends(get_session)):
    ...
    if ght_ctx:
        ...
        ejs = session.exec(
            select(EntiteJuridique).where(EntiteJuridique.is_active.is_(True))
        ).all()
    ...
    return get_templates_with_filters(request).TemplateResponse(
        request, "endpoint_detail.html", ctx
    )
...
def create_endpoint(
    ...
    if ej_id and ght_id:
        ...
        if ej_obj and ej_obj.ght_context_id != ght_id:
            raise HTTPException(
                status_code=400,
                detail="L'établissement choisi n'appartient pas au GHT sélectionné",
            )
    ...
    e = SystemEndpoint(
        ...
        kind=kind.upper(),
        role=role,
        is_enabled=_bool_from_str(is_enabled, True),
        target_system_key=target_system_key.strip() or None
    ...
    e.linked_endpoint_id = _parse_optional_form_id(
...
def update_endpoint(
    ...
    ej_id = (
        int(entite_juridique_id)
        if entite_juridique_id and entite_juridique_id.strip()
        else None
    )
    ...
    if not ght_id and not ej_id:
        ...
        ghts = session.exec(
            select(GHTContext).where(GHTContext.is_active.is_(True))
        ).all()
        ejs = session.exec(
            select(EntiteJuridique).where(EntiteJuridique.is_active.is_(True))
        ).all()
        ...
        is_running = (
            e.is_enabled
            if uses_background_poller(e.kind)
            else endpoint_id in set(registry.running_ids())
        )
        return get_templates_with_filters(request).TemplateResponse(
            request,
            "endpoint_detail.html",
    ...
    e.target_system_key = (
        target_system_key.strip() or None if target_system_key else None
    )
    e.ght_context_id = ght_id
    e.entite_juridique_id = ej_id
    e.linked_endpoint_id = (
        int(linked_endpoint_id)
        if linked_endpoint_id and linked_endpoint_id.strip()
        else None
    )
    ...
    e.ftp_remote_inbox_path, e.ftp_remote_outbox_path = (
        ftp_remote_inbox_path,
        ftp_remote_outbox_path,
    )
    e.ftp_remote_archive_path, e.ftp_remote_error_path = (
        ftp_remote_archive_path,
        ftp_remote_error_path,
    )
...
def start_endpoint(endpoint_id: int, session=Depends(get_session)):
    ...
    return RedirectResponse(
        url=f"/endpoints/{endpoint_id}", status_code=status.HTTP_303_SEE_OTHER
    )
...
def show_clone_structure_form(
    endpoint_id: int, request: Request, session: Session = Depends(get_session)
):
    ...
    return get_templates_with_filters(request).TemplateResponse(
        request, "endpoint_clone_structure.html", {"source": source, "targets": targets}
    )


@router.post("/{source_id}/clone-structure/{target_id}")
def clone_structure(
    source_id: int, target_id: int, session: Session = Depends(get_session)
):
    ...
    try:
        ...
        source_context = session.exec(
            select(EndpointContext).where(EndpointContext.endpoint_id == source_id)
        ).first()
        ...
        target_context = session.exec(
            select(EndpointContext).where(EndpointContext.endpoint_id == target_id)
        ).first()
        ...
        source_mappings = session.exec(
            select(PatientContextMapping).where(
                PatientContextMapping.context_id == source_context.id
            )
        ).all()
        ...
        source_mappings = session.exec(
            select(DossierContextMapping).where(
                DossierContextMapping.context_id == source_context.id
            )
        ).all()
        ...
        source_mappings = session.exec(
            select(VenueContextMapping).where(
                VenueContextMapping.context_id == source_context.id
            )
        ).all()
        ...
        source_mappings = session.exec(
            select(MouvementContextMapping).where(
                MouvementContextMapping.context_id == source_context.id
            )
        ).all()
        ...
        return RedirectResponse(
            url=f"/endpoints/{target_id}/context", status_code=status.HTTP_303_SEE_OTHER
        )
...
def show_endpoint_context(
    ...
    context = session.exec(
        select(EndpointContext).where(EndpointContext.endpoint_id == endpoint_id)
    ).first()

    if context:
        patient_mappings = session.exec(
            select(PatientContextMapping).where(
                PatientContextMapping.context_id == context.id
            )
        ).all()
        dossier_mappings = session.exec(
            select(DossierContextMapping).where(
                DossierContextMapping.context_id == context.id
            )
        ).all()
        venue_mappings = session.exec(
            select(VenueContextMapping).where(
                VenueContextMapping.context_id == context.id
            )
        ).all()
        mouvement_mappings = session.exec(
            select(MouvementContextMapping).where(
                MouvementContextMapping.context_id == context.id
            )
        ).all()
    else:
        patient_mappings = []
...
def execute_scenario_on_endpoint(
    endpoint_id: int,
    scenario_id: int,
    request: Request,
    ...
    try:
        ...
        flash(
            request,
            f"Jeu {play.play_key} : {play.status}",
    ...
    return RedirectResponse(
        url=f"/endpoints/{endpoint_id}/scenarios", status_code=status.HTTP_303_SEE_OTHER
    )
