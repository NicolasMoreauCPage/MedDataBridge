...
class FilePollerService:
    ...
    def _detect_encoding(file_path: Path) -> str:
        ...
        try:
            ...
            if match:
                ...
                logger.error(
                    f"[ENCODING] {file_path.name}: detected encoding={encoding}"
                )
                return encoding
        except Exception as e:
            logger.error(
                f"[ENCODING] Error detecting encoding for {file_path.name}: {e}"
            )
        # Encodage par défaut
    ...
    async def _scan_endpoint(self, endpoint: SystemEndpoint):
        ...
        # SFTP endpoint
        if (
            getattr(endpoint, "ftp_use_sftp", False)
            and endpoint.ftp_host
            and endpoint.ftp_remote_inbox_path
        ):
            logger.info(
                f"[SFTP] Scanning SFTP endpoint: {endpoint.ftp_host}:{endpoint.ftp_port or 22}{endpoint.ftp_remote_inbox_path}"
            )
            reader = SFTPReader(
                ...
                for filename in files:
                    try:
                        ...
                        await self._process_message(
                            content, DummyPath(filename), endpoint
                        )
                        ...
                        logger.error(
                            f"[SFTP] Error processing {filename}: {e}", exc_info=True
                        )
    ...
    async def _process_all_files_async(self, reader: FileSystemReader, callback):
        ...
        for file_path in files:
            ...
            try:
                ...
                processing_path = file_path.with_suffix(
                    file_path.suffix + ".processing"
                )
                ...
                if success:
                    ...
                    if reader.archive_path:
                        ...
                        archived_name = (
                            file_path.name
                        )  # Original name without .processing
                ...
                logger.error(
                    f"[FILE_ERROR] Error processing {file_path}: {e}", exc_info=True
                )
    ...
    async def _process_message(
        self, content: str, file_path: Path, endpoint: SystemEndpoint
    ) -> bool:
        """
        ...
        with SQLModelSession(engine) as msg_session:
            try:
                ...
                has_hprim = (
                    "hprimxml" in content_lower
                    or "hprim.org" in content_lower
                    or "<evenementsserveuractes" in content_lower
                    or "<evenementspms" in content_lower
                    or "<evenementsserveurétatspatient" in content_lower
                )
                ...
                logger.error(
                    f"[DETECTION] File={file_path.name} is_xml={is_xml} has_hprim={has_hprim} is_hprim={is_hprim}"
                )
                if is_xml:
                    logger.error(
                        f"[DETECTION] {file_path.name} XML content check: lower_has_hprimxml={'hprimxml' in content_lower}, has_hprim_org={'hprim.org' in content_lower}, has_evenements={'evenements' in content_lower[:500]}"
                    )

                if is_hprim:
                    # Handle HPRIM XML message
                    return await self._handle_hprim(
                        content, file_path, msg_session, endpoint
                    )
                ...
                if msg_log:
                    ...
                    msg_log.message_type = (
                        f"{details['message_code']}^{details['trigger_event']}"
                        if details["trigger_event"]
                        else details["message_code"]
                    )
                    ...
                    msg_log = MessageLog(
                        direction="in",
                        kind="HL7",
                        message_type=f"{details['message_code']}^{details['trigger_event']}"
                        if details["trigger_event"]
                        else details["message_code"],
                ...
                if category == "MFN":
                    ...
                    return await self._handle_adt(
                        content, msg_log, msg_session, endpoint
                    )
                else:
                    self.stats["unknown_messages"] += 1
                    logger.warning(
                        f"Unknown message category '{category}' for file {file_path.name}. Message type: {details.get('message_code')}, trigger: {details.get('trigger_event')}"
                    )
                ...
                logger.error(
                    f"Error processing message {file_path.name}: {e}", exc_info=True
                )
                try:
    ...
    async def _handle_hprim(
        self, content: str, file_path: Path, session: Session, endpoint: SystemEndpoint
    ) -> bool:
        """
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"HPRIM msg_log commit failed: {commit_error}", exc_info=True
                )
                session.rollback()
                raise
            ...
            if not result.get("succes"):
                ...
                try:
                    if err_type == "XSD_VALIDATION" and result.get("erreurs"):
                        ...
                        first_err = (
                            result["erreurs"][0].get("message")
                            if isinstance(result["erreurs"][0], dict)
                            else str(result["erreurs"][0])
                        )
                    else:
                ...
                try:
                    ...
                    logger.error(
                        f"HPRIM error commit failed: {commit_error}", exc_info=True
                    )
                    session.rollback()
            ...
            schema_info = result.get("schema_utilise")
            msg_log.ack_payload = (
                f"HPRIM validated OK{(' (' + schema_info + ')') if schema_info else ''}"
            )
            session.add(msg_log)
            try:
                session.commit()
                ...
                logger.error(
                    f"HPRIM final commit failed: {commit_error}", exc_info=True
                )
                session.rollback()
                raise
            ...
            try:
                ...
                if existing_log:
                    ...
                    msg_log = MessageLog(
                        ...
                        endpoint_id=endpoint.id,
                        correlation_id=correlation_id
                        or f"HPRIM_ERROR_{datetime.utcnow().isoformat()}",
                        status="error",
                        payload=content,
    ...
    async def _handle_adt(
        ...
        try:
            ...
            ack = await on_message_inbound_async(
                content, session, endpoint, existing_log=msg_log
            )
            # Parse HL7 ACK for AE/AR codes (negative ACK)
            msa = None
            ack_code = "AA"
...
def _write_ack_to_file(
    self, endpoint: SystemEndpoint, correlation_id: str, ack_content: str
) -> bool:
    """
    ...
    try:
        if not endpoint.ack_path:
            logger.debug(
                f"No ack_path configured for endpoint {endpoint.id}, skipping ACK write"
            )
