...
def list_ej_configs(request: Request, session: Session = Depends(get_session)):
    ...
    for ej in ejs:
        ...
        ej_data.append({"ej": ej, "config": config, "has_config": config is not None})
...
def edit_ej_config(
    request: Request, ej_id: int, session: Session = Depends(get_session)
):
    ...
    if config:
        for field in [
            "uf_hospitalisation_id",
            "uf_consultation_id",
            "uf_urgences_id",
            "uf_mutation_cible_id",
        ]:
...
def save_ej_config(
    ...
    return RedirectResponse(
        url=f"/config/scenario-ej?success=1&ej={ej.name}", status_code=303
    )


@router.delete("/{ej_id}", name="scenario_ej_config_delete")
def delete_ej_config(ej_id: int, session: Session = Depends(get_session)):
...
def get_ej_structure_api(ej_id: int, session: Session = Depends(get_session)):
...
def get_ej_structure(ej_id: int, session: Session) -> dict:
    ...
    for eg in egs:
        ...
        poles = session.exec(
            select(Pole).where(Pole.entite_geo_id == eg.id).order_by(Pole.name)
        ).all()
        ...
        for pole in poles:
            ...
            services = session.exec(
                select(Service).where(Service.pole_id == pole.id).order_by(Service.name)
            ).all()
        ...
        egs_data.append(
            {
                "eg": {"id": eg.id, "name": eg.name, "identifier": eg.identifier},
                "poles": poles_data,
            }
        )

    return {
        "ej": {"id": ej.id, "name": ej.name, "identifier": ej.identifier},
        "egs": egs_data,
