...
from app.schemas.clinical_api import (
    DossierRead,
    DossierUpdate,
    MouvementRead,
    VenueRead,
...
def get_dossier(dossier_id: int, session: Session = Depends(get_session)):
    """
...
@router.put("/{dossier_id}", response_model=DossierRead)
def update_dossier(
    dossier_id: int,
...
def delete_dossier(dossier_id: int, session: Session = Depends(get_session)):
    """
...
@router.get("/{dossier_id}/venues", response_model=List[VenueRead])
def get_dossier_venues(dossier_id: int, session: Session = Depends(get_session)):
    """
...
@router.get("/{dossier_id}/mouvements", response_model=List[MouvementRead])
def get_dossier_mouvements(dossier_id: int, session: Session = Depends(get_session)):
    """
    ...
    query = (
        select(Mouvement)
        .where(Mouvement.dossier_id == dossier_id)
        .order_by(Mouvement.mouv_date)
    )
    return session.exec(query).all()


@router.post("/{dossier_id}/close", response_model=DossierRead)
def close_dossier(
    dossier_id: int,
    date_end: Optional[datetime] = None,
    session: Session = Depends(get_session),
