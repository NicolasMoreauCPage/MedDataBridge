...
Ce document décrit de manière exhaustive l’intégration des messages IHE PAM (ADT, ZBE, MRG, etc.) dans MedData_Bridge, en conformité avec les spécifications IHE France (voir `/docs/SpecIHEPAM/`). Il s’adresse aux experts de l’interopérabilité, développeurs et intégrateurs qui doivent maintenir, étendre ou valider le pipeline PAM.
...
- Les identifiants CX/EI sont validés selon les contraintes IHE France (voir `/docs/SpecIHEPAM/IHE_France_Constraints_on_HL7_data_types_for_ITI_V1.8.1.txt`).
...
- Les conventions de codage sont alignées sur `/docs/SpecIHEPAM/IHE_France_Constraints_on_HL7_data_types_for_ITI_V1.8.1.txt`.
# 
...
- **Listes de valeurs officielles** : genre, classe patient, nature mouvement, status, triggers, etc. (voir `/docs/SpecIHEPAM/Publication-IHE_FRANCE_PAM_National_Extension_v2.11.1.txt`).
