...
## Analyse de la mise en œuvre actuelle

### Contrôles présents (récapitulatif)
...
1. Contrôles récemment ajoutés (delta)
...
## Contrôles BP6 manquants ou à renforcer

### Stateless (par-message)
...
1. Stateful / séquence (nécessitent historique DB) — prioritaires
...
1. Phase 1 — contrôles per-message (stateless) à finaliser
...
1. Phase 2 — validateur stateful (séquence) — implémentation recommandée
...
## Contrôles à implémenter prioritairement

1. ZBE-6 existence & consistency check (stateful) — HIGH
1. Allowed transition check against persisted last event for the venue (stateful) — HIGH
1. PID-32 enforcement rules (stateless but business-sensitive) — MEDIUM
1. PV1 reservation/occupation checks (stateful) — MEDIUM
1. PV1-36 / PV1-41 scenario checks (stateless partly, stateful for venue closure) — MEDIUM
