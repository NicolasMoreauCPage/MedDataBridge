...
from app.schemas.clinical_api import DossierRead, PatientRead
...
def create_patient_endpoint(
    patient: PatientCreateSchema, session: Session = Depends(get_session)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erreur lors de la création du patient: {str(e)}"
        )
...
@router.get("/{patient_id}/dossiers", response_model=List[DossierRead])
def get_patient_dossiers(patient_id: int, session: Session = Depends(get_session)):
    """
...
def merge_patients(
    patient_id: int, other_id: int, session: Session = Depends(get_session)
):
    """
    ...
    return {
        "message": f"Patient {other_id} fusionné dans {patient_id}",
        "moved_dossiers": len(dossiers),
    }
