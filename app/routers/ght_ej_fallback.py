"""Router fallback pour la page détail Entité Juridique.

Motivation: certaines routes de `ght.py` ne sont pas chargées à cause
probable d'une interruption d'import. Ce fallback garantit la présence
de /admin/ght/{context_id}/ej/{ej_id}.
"""
...
router = APIRouter(prefix="/ght", tags=["ght-fallback"])
...
async def ej_detail(
    ...
    counts = {
        ...
        "entites_geo_actives": len(geo_ids),
        "poles": len(pole_ids),
