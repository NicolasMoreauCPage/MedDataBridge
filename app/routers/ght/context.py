...
def edit_ght_context_form(
    request: Request, context_id: int, session: Session = Depends(get_session)
):
    ...
    return templates.TemplateResponse(request, "ght_form.html", {"context": context})
...
def update_ght_context(
    ...
    if code != context.code:
        ...
        if existing:
            flash(
                request, "Ce code est déjà utilisé par un autre contexte GHT.", "error"
            )
            accept = request.headers.get("accept", "")
            if "application/json" in accept:
                return {
...
def _test_set_session(
    request: Request,
    body_raw: bytes = Depends(read_body),
    session: Session = Depends(get_session),
):
    ...
    try:
        ...
        # Solution de repli pour le formulaire auto-soumis par la page HTML.
        body = dict(parse_qsl(body_raw.decode("utf-8", errors="replace")))
    ...
    if ght_code and not ght_id:
        found = session.exec(
            select(GHTContext).where(GHTContext.code == str(ght_code))
        ).first()
        if found:
            ght_id = found.id
        else:
            ...
            try:
                new_ctx = GHTContext(
                    name=str(ght_code), code=str(ght_code), is_active=True
                )
                session.add(new_ctx)
                session.commit()
    ...
    try:
        ...
        try:
            ...
            if ght_id is not None:
                payload["ght_id"] = (
                    int(ght_id)
                    if isinstance(ght_id, (int, str)) and str(ght_id).isdigit()
                    else ght_id
                )
            if ej_id is not None:
                payload["ej_id"] = (
                    int(ej_id)
                    if isinstance(ej_id, (int, str)) and str(ej_id).isdigit()
                    else ej_id
                )
            if payload:
                resp.set_cookie(
...
def _test_set_session_get(
    request: Request,
    token: str | None = None,
    ght_id: int | None = None,
    ej_id: int | None = None,
    ght_code: str | None = None,
    session: Session = Depends(get_session),
):
    ...
    if not (
        os.getenv("TESTING", "0") in ("1", "true", "True")
        or (env_token and token == env_token)
    ):
        raise HTTPException(status_code=404, detail="Not found")
    ...
    js_lines.append(
        "document.cookie = 'medbridge_test_data=' + encodeURIComponent(JSON.stringify(payload)) + '; path=/; SameSite=Lax';"
    )
    if ght_id is not None:
        js_lines.append(
            f"document.cookie = 'ght_context_id={ght_id}; path=/; SameSite=Lax';"
        )
    if ej_id is not None:
        js_lines.append(
            f"document.cookie = 'ej_context_id={ej_id}; path=/; SameSite=Lax';"
        )
    ...
    # /admin/ght.
    js_lines.append(
        "var iframe = document.createElement('iframe'); iframe.style.display='none'; iframe.name='session_iframe'; document.body.appendChild(iframe);"
    )
    js_lines.append(
        "var form = document.createElement('form'); form.method='POST'; form.action='/admin/ght/_test/session'; form.target='session_iframe';"
    )
    if ght_id is not None:
        js_lines.append(
            f"var i = document.createElement('input'); i.type='hidden'; i.name='ght_id'; i.value='{ght_id}'; form.appendChild(i);"
        )
    if ej_id is not None:
        js_lines.append(
            f"var j = document.createElement('input'); j.type='hidden'; j.name='ej_id'; j.value='{ej_id}'; form.appendChild(j);"
        )
    if ght_code is not None:
        js_lines.append(
            f"var k = document.createElement('input'); k.type='hidden'; k.name='ght_code'; k.value='{ght_code}'; form.appendChild(k);"
        )
    js_lines.append("document.body.appendChild(form); form.submit();")
    js_lines.append(
        "iframe.onload = function(){ window.location.href = '/admin/ght'; };"
    )
    js_lines.append("})();")
    body = (
        """
...
def _test_get_session_debug_html(request: Request, token: str | None = None):
