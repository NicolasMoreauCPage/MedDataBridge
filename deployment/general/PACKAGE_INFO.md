...
Le répertoire `deployment/` contient **TOUT** ce qui est nécessaire pour déployer MedData Bridge sur un serveur Fedora 7.9 **SANS connexion Internet** avec Python 3.8 et **SQLite**.
...
tar -czf meddata-bridge-sqlite-$(date +%Y%m%d).tar.gz deployment/
...
Le package `deployment/` est **complet et optimisé pour SQLite** - déploiement sur Fedora 7.9 sans connexion Internet.
